...
## Spatial Joins
```{r}
# `st_join` by default uses a left join (can only do left or inner join)
...
seine_buffer_union <- st_union(seine_buffer)

tm_shape(seine_buffer_union) +
  tm_polygons() +
tm_shape(seine) +
  tm_lines() 
